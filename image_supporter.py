...
class ImageFormatter:
    def __init__(self, default_image_type):
        self.__default_image_type = default_image_type

    def transform_image(self, image_path):
        with Image.open(image_path) as img:
            print("original image format: ", img.format)
            img_rgb = img.convert('RGB')
            new_image_path = image_path.split(".")[0] + "." + self.__default_image_type
            img_rgb.save(new_image_path, format=self.__default_image_type)
